...
pub fn parse_zip(fa: &mut FileAnalyze) -> bool {
    ...
    if sig != 0x0304 && sig != 0x0102 && sig != 0x0506 { return false; }
...
pub fn parse_rar(fa: &mut FileAnalyze) -> bool {
    ...
    let buf = fa.peek_raw(remain).map(|b| b.to_vec());
    let Some(buf) = buf else { return false };
    if buf.len() < 7 || &buf[0..4] != b"Rar!" { return false; }
...
pub fn parse_tar(fa: &mut FileAnalyze) -> bool {
    ...
    let buf = fa.peek_raw(remain).map(|b| b.to_vec());
    let Some(buf) = buf else { return false };
    if buf.len() < 257 { return false; }

    // UStar magic at offset 257
    let magic = if buf.len() > 263 { &buf[257..263] } else { return false; };
    ...
    if !is_ustar {
        ...
        if checksum.is_empty() || checksum.len() > 8 { return false; }
        ...
        if computed != parsed { return false; }
    }
...
pub fn parse_gzip(fa: &mut FileAnalyze) -> bool {
    ...
    let buf = fa.peek_raw(remain).map(|b| b.to_vec());
    let Some(buf) = buf else { return false };
    if buf.len() < 2 || buf[0] != 0x1F || buf[1] != 0x8B { return false; }
...
pub fn parse_bzip2(fa: &mut FileAnalyze) -> bool {
    ...
    let buf = fa.peek_raw(remain).map(|b| b.to_vec());
    let Some(buf) = buf else { return false };
    if buf.len() < 3 || buf[0] != 0x42 || buf[1] != 0x5A || buf[2] != 0x68 { return false; }
    // BZh + version digit
...
pub fn parse_iso9660(fa: &mut FileAnalyze) -> bool {
    ...
    if buf.len() < magic_offset + 5 { return false; }
    if &buf[magic_offset..magic_offset + 5] != b"CD001" { return false; }

    let sys_id = std::str::from_utf8(&buf[magic_offset - 1 + 8..magic_offset - 1 + 40]).unwrap_or("").trim().to_string();
    let vol_id = std::str::from_utf8(&buf[magic_offset - 1 + 40..magic_offset - 1 + 72]).unwrap_or("").trim().to_string();

    let mut extras = Vec::new();
    if !sys_id.is_empty() { extras.push(("System_ID", sys_id.as_str())); }
    if !vol_id.is_empty() { extras.push(("Volume_ID", vol_id.as_str())); }
...
pub fn parse_elf(fa: &mut FileAnalyze) -> bool {
    ...
    let buf = fa.peek_raw(remain).map(|b| b.to_vec());
    let Some(buf) = buf else { return false };
    if buf.len() < 16 || buf[0] != 0x7F || &buf[1..4] != b"ELF" { return false; }
    ...
    let type_str = match elf_type { 1 => "Relocatable", 2 => "Executable", 3 => "Shared", _ => "Unknown" };
    let mach_str = match machine {
        3 => "x86", 62 => "x86-64", 40 => "ARM", 183 => "AArch64",
        8 => "MIPS", 21 => "PowerPC", _ => "Unknown",
    };
...
pub fn parse_mach_o(fa: &mut FileAnalyze) -> bool {
    ...
    fill_archive(fa, "Mach-O", &[
        ("Format_Profile", bits),
        ("Format_Settings_Endianness", endian),
    ]);
    true
}
...
pub fn parse_mz_exe(fa: &mut FileAnalyze) -> bool {
    ...
    let buf = fa.peek_raw(remain).map(|b| b.to_vec());
    let Some(buf) = buf else { return false };
    if buf.len() < 2 || buf[0] != 0x4D || buf[1] != 0x5A { return false; }
    ...
    let format = if pe_offset > 0 && pe_offset + 4 <= buf.len() && &buf[pe_offset..pe_offset + 4] == b"PE\0\0" {
        "Windows PE"
    } else {
        "MZ DOS"
    };
...
mod tests {
    ...
    fn elf_detects_magic() {
        ...
        buf[4] = 2; // 64-bit
        buf[5] = 1; // LE
        buf[16] = 3; buf[17] = 0; // shared
        buf[18] = 62; buf[19] = 0; // x86-64
        let mut fa = FileAnalyze::new(&buf);
