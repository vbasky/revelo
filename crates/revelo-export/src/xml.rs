...
fn emit_stream_fields(out: &mut String, kind: StreamKind, stream: &revelo_core::Stream) {
...
pub(crate) fn extra_field_order(kind: StreamKind) -> &'static [&'static str] {
    match kind {
        ...
        // field.
        StreamKind::Video => &["CodecConfigurationBox"],
        _ => &[],
    }
}
...
pub(crate) fn canonical_field_order(kind: StreamKind) -> &'static [&'static str] {
    match kind {
        ...
        // Mirrors MediaInfoLib's Video stream field order. Verified against
        // the oracle for AVC/HEVC/VP9 in MP4/MOV/MKV: the Format_Settings_*
        // and Format_Level/Tier group sit right after Format_Profile;
        // Sampled_*/PixelAspectRatio precede DisplayAspectRatio; FrameRate_
        // Num/Den follow FrameRate; Encoded_Library* come after StreamSize;
        // the colour group (with MKV _Source variants) trails Default/Forced.
        // CodecConfigurationBox is emitted in <extra> (see extra_field_order).
        StreamKind::Video => &[
            "Count",
            "StreamCount",
            "StreamKind",
