...
mod tests {
    ...
    #[test]
    fn parse_tags_keeps_embedded_metadata_for_unlisted_formats() {
        let buf =
            br#"prefix xmpmeta <rdf:RDF><xmp:CreatorTool>Camera</CreatorTool></rdf:RDF> suffix"#;
        let mut fa = FileAnalyze::new(buf);
        fa.stream_prepare(StreamKind::General);
        fa.set_field(StreamKind::General, 0, "Format", "PDF");
...
fn should_scan_embedded_metadata(fa: &FileAnalyze) -> bool {
    ...
    if fa.stream_count(StreamKind::Video) > 0 || fa.stream_count(StreamKind::Audio) > 0 {
        return false;
    }
