...
pub fn parse_adm(fa: &mut FileAnalyze) -> bool {
    ...
    if &buf[0..4] != b"ADM " && !buf.windows(4).any(|w| w == b"axml") { return false; }
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test() { let buf = b"ADM \x00\x00\x00\x00".to_vec(); let mut fa = FileAnalyze::new(&buf); assert!(parse_adm(&mut fa)); }
}
