...
use mediainfo_parsers_image::{parse_bmp, parse_dds, parse_dpx, parse_gif, parse_ico, parse_jpeg, parse_png, parse_psd, parse_tiff, parse_webp};
...
fn run_rust_engine(path: &str) -> Result<String, String> {
    ...
    let parsers: [(&str, fn(&mut FileAnalyze) -> bool); 22] = [
        ("WAV", parse_wav),
        ("AVI", parse_avi),
        ("WebP", parse_webp),
        ("AIFF", parse_aiff),
