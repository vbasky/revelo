...
fn run_rust_engine(path: &str) -> Result<String, String> {
    ...
    let parsers: [(&str, fn(&mut FileAnalyze) -> bool); 109] = [
        ("WAV", parse_wav),
        ("AVI", parse_avi),
        ("CDXA", parse_cdxa),
        ("AMV", parse_amv),
