        ...
        match track.track_type {
            ...
            Some(1) => {
                ...
                if track.codec_id.as_deref() == Some("V_AV1") {
                    if let Some(ref private) = track.codec_private {
                        if let Some(info) = mediainfo_parsers_video::parse_av1_from_codec_config(private) {
                        ...
                        if private.len() >= 4 && private[0] == 1 {
                            ...
                            if private.len() >= 8 {
                                ...
                                for _ in 0..num_sps {
                                    ...
                                    if let Some(sps_info) = mediainfo_parsers_video::parse_avc_sps(sps_data) {
                                        if sps_info.width > 0 {
                        ...
                        if private.len() >= 23 && private[0] == 1 {
                            ...
                            if private.len() > 23 {
                                ...
                                if let Some(sps) = sps_data {
                                    if let Some(sps_info) = mediainfo_parsers_video::parse_hevc_sps(&sps) {
                                        if sps_info.width > 0 {
                                            fa.Fill(StreamKind::Video, pos, "Width", sps_info.width.to_string(), true);
                                        }
                                        if sps_info.height > 0 {
                                ...
                                if !sei_nalus.is_empty() {
                                    ...
                                    if let Some(encoder) = mediainfo_parsers_video::extract_encoder_from_sei_nalus(&refs) {
