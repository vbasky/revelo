...
pub fn to_mpeg7(streams: &StreamCollection, file_path: &str) -> String {
    ...
    for kind in [StreamKind::General, StreamKind::Video, StreamKind::Audio] {
        for p in 0..streams.count_get(kind) { if let Some(s) = streams.stream(kind, p) {
            ...
            for (k, v) in s.iter() { out.push_str(&format!("<{k}>{v}</{k}>\n", k=k, v=v.as_str())); }
            out.push_str(&format!("</{kind:?}>\n", kind=kind));
        }}
    }
...
#[cfg(test)] mod tests { use super::*; use zenlib::Ztring;
    #[test] fn test() { let mut c = StreamCollection::new(); c.fill(StreamKind::General, 0, "Format", Ztring::from("MP4"), false); let xml = to_mpeg7(&c, "/x.mp4"); assert!(xml.contains("Mpeg7")); }
}
